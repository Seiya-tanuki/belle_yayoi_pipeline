...
- Debit/credit default: 蛟滓婿=莉ｮ謇暮≡, 雋ｸ譁ｹ=迴ｾ驥・(belle_yayoi_buildRow)
- Debit tax default: BELLE_FALLBACK_DEBIT_TAX_KUBUN_DEFAULT (default: 蟇ｾ雎｡螟・
...
- No/invalid date -> use fiscal end, RID=DATE_FALLBACK, DT=NO_DATE, FIX=隱､縺｣縺溷叙蠑墓律.
...
3) line_items description with tax amount (蜀・ｶ郁ｲｻ遞守ｭ・縺・■豸郁ｲｻ遞・etc)
...
- From 2019-10-01 and later, 8% should use "隱ｲ蟇ｾ莉募・霎ｼ霆ｽ貂・%" in tax kubun notation.
- Source: Yayoi Kaikei Next import format (tax kubun) lists 8% reduced as "霆ｽ貂・%".
- Invoice suffix (驕ｩ譬ｼ) can be appended, but may be disabled via BELLE_FALLBACK_APPEND_INVOICE_SUFFIX.
...
- BELLE_FALLBACK_DEBIT_TAX_KUBUN_DEFAULT (default: 蟇ｾ雎｡螟・
